...
import com.example.hatice.project.BusinessG.InviteeListAdapter;
import com.example.hatice.project.BusinessG.SendSMS;
import com.example.hatice.project.ModelG.Contact;
import com.example.hatice.project.ModelG.ContactListSingleton;
import com.example.hatice.project.ModelG.Invitee;
import com.example.hatice.project.ModelG.InviteeListSingleton;
import com.example.hatice.project.ModelG.InviteeStatus;
        ...
        deleteInvitee.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                for(int i=0; i < invitee_expandable.getChildCount(); i++) {
                    ...
                    if (cb.isChecked()) {
                        checkedIdList.add((String) tv.getText());
                        checkedInvitees.add(InviteeListSingleton.getInst().getInviteeList().get(i));
                    ...
                    if(checkedIdList!=null && checkedIdList.size()>0){
                            ...
                            removeCheckedDialog.setPositiveButton(
                                    ...
                                    new DialogInterface.OnClickListener() {
                                        public void onClick(DialogInterface dialog, int id) {
                                            ContactListSingleton.getInst().getSelectedContactsList().removeContactsByIds(checkedIdList);
                                            InviteeListSingleton.getInst().getInviteeList().removeAll(checkedInvitees);
    ...
    private void convertContactsToInvitees() {
        ...
        for (Contact c: ContactListSingleton.getInst().getLastSelectedContactsList().contactArrayList) {
            if(InviteeListSingleton.getInst().isSameNumber(c.getPhoneNumber())){
                ...
                Invitee in = new Invitee(c.getId(), c.getName(), c.getPhoneNumber(), null);
